...
func SetCpuCGroup(cpuPath string, pid int, res *ResourceConfig) error {
	if err := os.MkdirAll(cpuPath, 0700); err != nil {
		return fmt.Errorf("create cgroup path fail err=%s", err)
	}
	if err := ioutil.WriteFile(path.Join(cpuPath, cpuCfsQuota), []byte(res.CpuSet), 0700); err != nil {
		return fmt.Errorf("write cpu quota us fail err=%s", err)
	}
	if err := ioutil.WriteFile(path.Join(cpuPath, cpuShares), []byte(res.CpuShare), 0700); err != nil {
		return fmt.Errorf("write cpu quota us fail err=%s", err)
	}
